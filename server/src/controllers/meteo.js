...
exports.meteo = async (req, res) => {
    res.res.status(404).json({
        code: 404,
        error: 'You need to indicate the insee of a city'
    });
}
exports.insee = async (req, res) => {
    ...
    const result = dbForecast.map(elem => {
        elem = { ...elem, details: JSON.parse(elem.details) };
        elem.weatherIcon = weatherUtils.getIconByCode(elem.details.weather);
        return elem;
    })
    res.json(result)
}